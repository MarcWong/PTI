z = 78;
img = imread(['/Users/marcWong/Data/ProcessedData/001A/coloredimg/layer' num2str(z) '/17.jpg']);
roi = imread(['/Users/marcWong/Data/ProcessedData/001A/roi/roi' num2str(z/6) '.jpg']);
...
r = sqrt(double(img(:,:,1)).^2+double(img(:,:,2)).^2 + double(img(:,:,3)).^2) .* 0.006;
rxy = sqrt(double(img(:,:,1)).^2+double(img(:,:,2)).^2).*0.006;
for x = 1:2:256
    for y = 1:2:256
        if (rxy(x,y) < 0.1)
            rxy(x,y) = 0.1;
